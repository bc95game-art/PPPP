...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild {
    ...
    public static final TextInputLayout.C02112 ACCESSIBILITY_DELEGATE = new TextInputLayout.C02112(1);
    ...
    public NestedScrollView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet, C0130R.attr.nestedScrollViewStyle);
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, SCROLLVIEW_STYLEABLE, C0130R.attr.nestedScrollViewStyle, 0);
    ...
    */
    public final void computeScroll() {
        ...
        if (!this.mScroller.isFinished()) {
            ...
            if (i3 <= 0 || BundleKt.getDistance(edgeEffect) == 0.0f) {
                if (i3 < 0 && BundleKt.getDistance(edgeEffect2) != 0.0f) {
                    float f = height;
                    round = Math.round(BundleKt.onPullDistance(edgeEffect2, (i3 * 4.0f) / f, 0.5f) * (f / 4.0f));
                    if (round != i3) {
                        edgeEffect2.finish();
                    }
                }
                ...
                round = Math.round(BundleKt.onPullDistance(edgeEffect, ((-i3) * 4.0f) / height, 0.5f) * ((-height) / 4.0f));
                if (round != i3) {
                    edgeEffect.finish();
                }
            }
            ...
            iArr2[1] = 0;
            dispatchNestedPreScroll(0, i3, iArr2, null, 1);
            i = i3 - iArr2[1];
        ...
        if (actionMasked != 0) {
            ...
            if (actionMasked == 1) {
                ...
                if (Math.abs(yVelocity) >= this.mMinimumVelocity) {
                    ...
                    } else if (BundleKt.getDistance(edgeEffect2) != 0.0f) {
                        int i2 = -yVelocity;
                ...
                if (findPointerIndex == -1) {
                    ...
                    if (BundleKt.getDistance(edgeEffect) != 0.0f) {
                        onPullDistance = -BundleKt.onPullDistance(edgeEffect, -height, x);
                        if (BundleKt.getDistance(edgeEffect) == 0.0f) {
                            edgeEffect.onRelease();
                        }
                    } else {
                        if (BundleKt.getDistance(edgeEffect2) != 0.0f) {
                            onPullDistance = BundleKt.onPullDistance(edgeEffect2, height, 1.0f - x);
                            if (BundleKt.getDistance(edgeEffect2) == 0.0f) {
                                edgeEffect2.onRelease();
                            }
                        }
                        ...
                        if (this.mIsBeingDragged) {
                            int scrollBy = scrollBy(i, (int) motionEvent.getX(findPointerIndex), 0, false);
