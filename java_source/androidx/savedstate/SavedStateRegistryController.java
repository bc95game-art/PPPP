    ...
    public final void performSave(Bundle bundle) {
        ...
        synchronized (savedStateRegistryImpl.lock) {
            for (Map.Entry entry : savedStateRegistryImpl.keyToProviders.entrySet()) {
                Navigation.m654putSavedStateimpl(bundleOf, (String) entry.getKey(), ((SavedStateRegistry.SavedStateProvider) entry.getValue()).saveState());
            }
        }
        if (!bundleOf.isEmpty()) {
            Navigation.m654putSavedStateimpl(bundle, "androidx.lifecycle.BundlableSavedStateRegistry.key", bundleOf);
        }
    }
}
